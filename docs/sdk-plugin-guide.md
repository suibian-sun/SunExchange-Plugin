...
这是 SunExchange 原生插件体系的开发方法。借鉴 FIN 插件框架的高自由度设计，但通信基础完全复用 SunExchange 自有的**鹊桥（Queqiao）协议**，无需额外基础设施。插件以独立进程运行，通过 WebSocket 与宿主双向通信。

> 适用对象：想在 SunExchange 生态内编写"进服验证、群服互通、经济联动、自动管理"等能力的开发者。若你只想把插件发布到统一插件市场，另见 [插件制作完整指南](plugin-development-guide.md)。SDK 源码与示例插件位于 `SunExchange` 主仓库的 `plugin/` 目录。

## 一、体系总览

```
┌─────────────────────────── SunExchange 主进程 ───────────────────────────┐
│  Web 管理端  ·  任务工作进程(minecraftbridge)  ·  鹊桥 WS 服务(每任务一个) │
└──────────────────────────────────────┬───────────────────────────────────┘
                                       │ ws://<host>:<WorkerPort>/minecraft/ws
┌──────────────────────────────────────▼───────────────────────────────────┐
│  插件宿主（sunexchange-plugin 二进制）                                     │
...
核心概念：**插件 = 一个实现 `sdk.Plugin` 接口的独立进程**，宿主负责加载它并给它注入一个 `sdk.Context`。所有能力都通过 `Context` 暴露。

## 二、快速开始

### 1. 引入 SDK

独立模块，低 Go 版本（1.18+）即可编译，可被任意插件仓库直接引用：

```bash
module myplugin
go 1.18
require sunexchange/plugin/sdk v0.0.0
replace sunexchange/plugin => /path/to/SunExchange/plugin
```
...
```bash
# 连接本地某任务的鹊桥（端口取任务 WorkerPort）
go run ./cmd/sunexchange-plugin \
...
可以监听任意事件，并用**优先级**控制执行顺序。返回 `true` 表示消费事件、阻止后续监听器（拦截链）。
...
直接向宿主发送任意 `kind + data`，可用于扩展协议、对接自有系统——Freedom 拉满：
...
SunExchange/plugin/
├── sdk/            # 插件开发接口（独立可引用）
│   ├── sdk.go       # Plugin 接口、Context、事件、命令、API、广播
│   └── errors.go    # 错误定义
├── host/           # 插件宿主
│   ├── host.go      # 生命周期管理、事件分发
│   ├── bridge.go    # 鹊桥 WebSocket 桥接（Adapter）
│   ├── loader.go    # 动态加载 .so
│   └── events.go    # 事件解码（map → 强类型）
├── examples/entry/ # 示例插件（进服验证 + 群服互通）
│   └── so/          # .so 导出包装（package main + Plugin 符号）
└── cmd/
    ├── sunexchange-plugin/  # 插件宿主二进制（连接鹊桥、加载插件）
    └── standalone/          # 演示宿主（无鹊桥，内存模拟）
```

## 六、测试

宿主自带单元测试，覆盖 `.so` 动态加载、事件解码分发（修复 panic 的关键用例）、插件 API 与版本校验：

```bash
cd SunExchange/plugin
go build ./... && go test ./host/ -v
```

运行演示宿主（无需连真实鹊桥，内存模拟事件）：
...
A：`.so` 必须由 `package main` 编译，并包含 `func Plugin() sdk.Plugin` 与 `func main(){}`。参见 `examples/entry/so/main.go`。
